<div class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center space-x-4">
            <span class="text-sm font-medium {{ $maintenanceMode ? 'text-red-600' : 'text-green-600' }}">
                {{ $maintenanceMode ? '🟢 Maintenance Aktif' : '⚫ Maintenance Nonaktif' }}
            </span>
        </div>
    </div>

    <!-- Status Maintenance -->
    <div class="bg-{{ $maintenanceMode ? 'red' : 'gray' }}-50 dark:bg-{{ $maintenanceMode ? 'red' : 'gray' }}-900/20 border border-{{ $maintenanceMode ? 'red' : 'gray' }}-200 dark:border-{{ $maintenanceMode ? 'red' : 'gray' }}-700 rounded-lg p-4 mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h3 class="text-lg font-semibold text-{{ $maintenanceMode ? 'red' : 'gray' }}-800 dark:text-{{ $maintenanceMode ? 'red' : 'gray' }}-200">
                    Status Maintenance Mode
                </h3>
                <p class="text-{{ $maintenanceMode ? 'red' : 'gray' }}-600 dark:text-{{ $maintenanceMode ? 'red' : 'gray' }}-400 mt-1">
                    @if($maintenanceMode)
                ...
                <div class="mt-3 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div class="flex items-center space-x-2">
                        ...
                        <div>
                            ...
                            <div class="text-gray-600 dark:text-gray-400">{{ \Carbon\Carbon::parse($maintenanceStart)->translatedFormat('d M Y H:i') }}</div>
                        </div>
                    </div>
                    <div class="flex items-center space-x-2">
                        ...
                        <div>
                            ...
                            <div class="text-gray-600 dark:text-gray-400">{{ \Carbon\Carbon::parse($maintenanceEnd)->translatedFormat('d M Y H:i') }}</div>
                        </div>
                    </div>
                    <div class="flex items-center space-x-2">
                        ...
                        <div>
                            ...
                            <div class="text-gray-600 dark:text-gray-400">{{ $this->getTimeRemaining() }}</div>
                        </div>
                    </div>
                </div>
                @endif
            ...
            <button wire:click="toggleMaintenance" 
                    ...
                    class="px-4 py-2 rounded-lg font-medium transition-colors {{ $maintenanceMode ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-red-600 hover:bg-red-700 text-white' }} disabled:opacity-50">
            ...
            <div class="space-y-4">
                ...
                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-3">
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="text-blue-600 dark:text-blue-400">{{ $this->getEstimatedDuration() }}</span>
                    </div>
                </div>
    ...
    <div class="mt-6 bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg p-6 text-white">
        <div class="flex items-center justify-between">
            ...
            <div class="text-right">
                <div class="text-2xl font-bold">{{ now()->translatedFormat('H:i:s') }}</div>
                <div class="text-blue-100 text-sm">{{ now()->translatedFormat('l, d F Y') }}</div>
            </div>
        </div>
    </div>
