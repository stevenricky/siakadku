        ...
        <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center w-full sm:w-auto">
            ...
            >
                ...
                <span wire:loading.remove wire:target="createBackup">
                    Buat Backup 
                    @if($backupType === 'sql_full')
                        (SQL Full)
                    @elseif($backupType === 'sql')
                        (SQL Data)
                    @else
                        (JSON)
                    @endif
    ...
    @foreach (['success' => 'green', 'error' => 'red', 'info' => 'blue'] as $type => $color)
        @if (session()->has($type))
            <div 
                ...
                class="bg-{{ $color }}-100 border border-{{ $color }}-400 text-{{ $color }}-700 px-4 py-3 rounded-lg mb-6 flex items-center justify-between"
            >
                <div class="flex items-center">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path 
                            ...
                            stroke-width="2" 
                            d="@if($type=='success')M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z
                            @elseif($type=='error')M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z
                            @else M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z @endif"
                        />
                    </svg>
                    <span>{{ session($type) }}</span>
                </div>
                <button @click="show = false" class="text-{{ $color }}-700 hover:text-{{ $color }}-900">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow mb-6">
        <div class="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                ...
                @else
                    Total {{ count($backups) }} backup ditemukan
                @endif
            </p>
        </div>
                            ...
                            @foreach ($backups as $backup)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-4 py-4">
                                        ...
                                        @elseif(($backup['type'] ?? '') === 'sql')
                                            @if(str_contains($backup['name'], 'full_backup'))
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-800 text-xs font-medium rounded-full dark:bg-gray-700 dark:text-gray-300">
                                                {{ strtoupper($backup['type'] ?? 'UNKNOWN') }}
                                            </span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-4 text-sm text-gray-900 dark:text-white">{{ $backup['size'] }}</td>
                                    <td class="px-4 py-4 text-sm text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($backup['date'])->translatedFormat('d M Y H:i') }}</td>
                                    <td class="px-4 py-4">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <button 
                                                x-on:click="if (confirm('Yakin ingin menghapus backup {{ $backup['name'] }}?')) { $wire.deleteBackup('{{ $backup['name'] }}') }"
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                            <div class="flex items-start justify-between mb-3">
                                <div class="flex items-center">
                                    ...
                                    @endif
                                    <div>
                                        <h4 class="text-sm font-medium text-gray-900 dark:text-white truncate max-w-[200px]">{{ $backup['name'] }}</h4>
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ \Carbon\Carbon::parse($backup['date'])->translatedFormat('d M Y H:i') }}</p>
                                    </div>
                                </div>
                                @if(($backup['type'] ?? '') === 'json')
                                    <span class="inline-flex items-center px-2 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full dark:bg-green-900 dark:text-green-200">
                            ...
                            <div class="flex items-center justify-between">
                                <p class="text-sm text-gray-600 dark:text-gray-400">Ukuran: {{ $backup['size'] }}</p>
                                <div class="flex items-center space-x-2">
                                    <button 
                                        wire:click="downloadBackup('{{ $backup['name'] }}')"
